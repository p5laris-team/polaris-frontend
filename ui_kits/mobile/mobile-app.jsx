/* ============================================================
   Polaris Mobile — Android Frame Demo App
   ============================================================ */
...
function MobileApp() {
  ...
  const [user] = React.useState({ nickname: "별따라걷기", email: "user@example.com" });
  const [selectedCharacterType, setSelectedCharacterType] = React.useState("nova");
  const [character, setCharacter] = React.useState({ type: "nova", name: "작은노바", fullness: 80, energy: 55, affection: 35, skin: null });
  const [wallet, setWallet] = React.useState({ starPieces: 240, history: [{ type: "초기 별조각 지급", amount: 240, timestamp: "2026-05-20 09:00" }] });
  ...
  const addWalletTx = (type, amount) => setWallet(w => ({ starPieces: w.starPieces + amount, history: [{ type, amount, timestamp: "2026-05-20 12:00" }, ...w.history] }));
  const advanceMission = () => {
    if (missionIndex >= 15) { setActiveMission(null); return; }
    setActiveMission(missionTemplates[missionIndex]);
    setMissionIndex(i => i + 1);
  };
  const rejectMission = () => {
    if (!activeMission) return;
    setMissionHistory(h => [{ ...activeMission, status: "REJECTED", timestamp: "방금" }, ...h]);
  ...
  const submitMissionAnswer = (answer) => {
    ...
    setMissionHistory(h => [{ ...mission, status: "COMPLETED", answer, timestamp: "방금" }, ...h]);
  ...
  const useCareAction = (actionType) => {
    ...
    if (actionType === "FEED") {
      const idx = inventory.findIndex(i => i.id === "food");
      if (idx >= 0) setInventory(items => items.filter((_, i) => i !== idx));
      else if (wallet.starPieces >= 3) addWalletTx("밥 주기", -3);
      else return { success: false, error: "별조각이나 밥 아이템이 부족해요." };
      setCharacter(c => ({ ...c, fullness: Math.min(100, c.fullness + 30) }));
      return { success: true, message: `${character.name}가 든든해졌어요. 포만감 +30` };
    }
    if (actionType === "PLAY") {
      const idx = inventory.findIndex(i => i.id === "toy");
      if (idx >= 0) setInventory(items => items.filter((_, i) => i !== idx));
      else if (wallet.starPieces >= 5) addWalletTx("놀아주기", -5);
      else return { success: false, error: "별조각이나 장난감 아이템이 부족해요." };
  ...
  const purchaseItem = (item, tabName) => {
    if (wallet.starPieces < item.price) return { success: false, error: `별조각이 ${item.price - wallet.starPieces}개 부족해요.` };
  ...
  const useInventoryItem = (id) => id === "food" ? useCareAction("FEED") : id === "toy" ? useCareAction("PLAY") : { success: false, error: "사용할 수 없는 아이템이에요." };
  const claimShareReward = () => {
    if (shareRewardClaimedToday) { showToast("오늘 공유 보상은 이미 받았어요."); return; }
  ...
  const renderAppView = () => {
    if (appView === "missionAnswer") return <MissionAnsweringScreen character={character} activeMission={pendingCompletedMission} onAnswerSubmit={submitMissionAnswer} onBack={() => setAppView("tabs")} />;
    if (appView === "missionResult") return <MissionResultScreen character={character} activeMission={pendingCompletedMission} earnedStarPieces={earnedStarPieces} totalStarPieces={wallet.starPieces} onGoHome={() => setAppView("tabs")} />;
    if (appView === "characterDetail") return <CharacterDetailScreen character={character} wallet={wallet} inventory={inventory} equippedSkin={character.skin} onUseCareAction={useCareAction} onBack={() => setAppView("tabs")} onShowToast={showToast} />;
    if (appView === "shop") return <ShopScreen wallet={wallet} onPurchaseItem={purchaseItem} onBack={() => setAppView("tabs")} onShowToast={showToast} onOpenInventory={() => setAppView("inventory")} />;
    if (appView === "inventory") return <InventoryScreen inventory={inventory} equippedSkin={character.skin} onEquipSkin={equipSkin} onUseItem={useInventoryItem} character={character} onBack={() => setAppView("shop")} onShowToast={showToast} />;
    if (appView === "wallet") return <WalletHistoryScreen wallet={wallet} onBack={() => setAppView("tabs")} />;
    if (appView === "share") return <ShareScreen character={character} equippedSkin={character.skin} onClaimShareReward={claimShareReward} onBack={() => setAppView("tabs")} onShowToast={showToast} />;
    if (appView === "notifications") return <NotificationBoxScreen notifications={notifications} onMarkAsRead={(id) => setNotifications(ns => ns.map(n => n.id === id ? { ...n, unread: false } : n))} onBack={() => setAppView("tabs")} />;
    if (appView === "attendance") return <AttendanceCalendarScreen attendanceDates={attendanceDates} hasCheckedToday={attendanceDates.includes(TODAY)} onCheckAttendance={checkAttendance} onBack={() => setAppView("tabs")} />;

    let content;
    if (tab === "home") {
      content = <HomeScreen character={character} wallet={wallet} activeMission={activeMission} dailyMissionIndex={missionIndex} equippedSkin={character.skin} attendanceStreak={attendanceStreak} onCompleteClick={completeMissionStart} onRejectClick={rejectMission} onOpenNotifications={() => setAppView("notifications")} onOpenWalletHistory={() => setAppView("wallet")} onOpenAttendance={() => setAppView("attendance")} onOpenShare={() => setAppView("share")} />;
    } else if (tab === "missions") {
      content = <MissionHistoryScreen missionHistory={missionHistory} />;
    } else {
      content = <MyPageScreen user={user} character={character} wallet={wallet} attendanceStreak={attendanceStreak} onLogout={() => { setFlow("login"); setTab("home"); setAppView("tabs"); }} />;
    }
    return (
  ...
  const renderContent = () => {
    ...
    if (flow === "charSelect") return <CharacterSelectScreen onBack={() => setFlow("login")} onNext={(id) => { setSelectedCharacterType(id); setFlow("charName"); }} />;
    if (flow === "charName") return <CharacterNameScreen character={selectedCharacterType} onBack={() => setFlow("charSelect")} onNext={(name) => { setCharacter({ type: selectedCharacterType, name, fullness: 80, energy: 55, affection: 35, skin: null }); setFlow("onboarding"); }} />;
    if (flow === "onboarding") return <OnboardingScreen character={character.type} name={character.name} onBack={() => setFlow("charName")} onDone={() => setFlow("app")} />;
    return renderAppView();
  };

  return (
    <>
      <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 32, background: tweaks.mode === "dark" ? "#1A150D" : "#ECE6D8" }}>
        <AndroidDevice width={412} height={892} dark={tweaks.mode === "dark"}>
          <div style={{ background: "var(--bg-1)", minHeight: "100%", display: "flex", flexDirection: "column", position: "relative" }}>
            {renderContent()}
